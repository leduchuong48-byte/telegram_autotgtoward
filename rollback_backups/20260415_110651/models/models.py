from sqlalchemy import create_engine, Column, Integer, String, Boolean, ForeignKey, Enum, UniqueConstraint, DateTime, inspect, text
from sqlalchemy.engine import make_url
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import relationship, sessionmaker
from enums.enums import ForwardMode, PreviewMode, MessageMode, AddMode, HandleMode, CompareMode
import logging
import os
...
def migrate_db(engine):
    ...
    try:
        with engine.begin() as connection:
            ...
            forward_rules_columns = {column['name'] for column in inspector.get_columns('forward_rules')} if 'forward_rules' in existing_tables else set()
                    ...
                    for rule in rules:
                        ...
                        if selected_types:
                            ...
                            media_types_data = {
                                ...
                                'text': 'text' in selected_types
                            }
                            
                            # 插入数据
                            connection.execute(
                                text("""
                                INSERT INTO media_types (rule_id, photo, document, video, audio, voice, text)
                                VALUES (:rule_id, :photo, :document, :video, :audio, :voice, :text)
                                """),
                                {
                                    'rule_id': rule_id,
    ...
    forward_rules_new_columns = {
        ...
        'media_type_filter_mode': 'ALTER TABLE forward_rules ADD COLUMN media_type_filter_mode VARCHAR DEFAULT "BLACKLIST"',
        ...
        'media_size_filter_mode': 'ALTER TABLE forward_rules ADD COLUMN media_size_filter_mode VARCHAR DEFAULT "less"',
        ...
        'enable_media_duration_filter': 'ALTER TABLE forward_rules ADD COLUMN enable_media_duration_filter BOOLEAN DEFAULT FALSE',
        'media_duration_minutes': 'ALTER TABLE forward_rules ADD COLUMN media_duration_minutes INTEGER DEFAULT 10',
        'media_duration_filter_mode': 'ALTER TABLE forward_rules ADD COLUMN media_duration_filter_mode VARCHAR DEFAULT "less"',
    ...
    with engine.begin() as connection:
        ...
        if 'media_types' in existing_tables and 'text' not in media_types_columns:
            try:
                connection.execute(text('ALTER TABLE media_types ADD COLUMN text BOOLEAN DEFAULT FALSE'))
                logging.info('已添加列: media_types.text')
            except Exception as e:
                logging.error(f'添加列 media_types.text 时出错: {str(e)}')
        ...
        # 统一 chats 表中的频道ID格式，避免 373... 与 -100373... 双记录导致规则不命中
        try:
            merged_count = 0
            normalized_count = 0

            legacy_rows = connection.execute(text("""
                SELECT id, telegram_chat_id, name, current_add_id
                FROM chats
                WHERE telegram_chat_id LIKE '-100%'
            """)).fetchall()
            ...
            for row in legacy_rows:
                ...
                legacy_id = row[1] or ''
                legacy_name = row[2]
                legacy_current_add_id = row[3]
                canonical_id = legacy_id[4:] if legacy_id.startswith('-100') else legacy_id

                if not canonical_id or not canonical_id.isdigit():
                    continue

                existing = connection.execute(
                    text("SELECT id, name, current_add_id FROM chats WHERE telegram_chat_id = :cid LIMIT 1"),
                    {"cid": canonical_id},
                ).fetchone()

                if existing and existing[0] != chat_id:
                    primary_id = existing[0]
                    primary_name = existing[1]
                    primary_current_add_id = existing[2]

                    connection.execute(
                        text("UPDATE forward_rules SET source_chat_id = :to_id WHERE source_chat_id = :from_id"),
                        {"to_id": primary_id, "from_id": chat_id},
                    )
                    connection.execute(
                        text("UPDATE forward_rules SET target_chat_id = :to_id WHERE target_chat_id = :from_id"),
                        {"to_id": primary_id, "from_id": chat_id},
                    )
                    ...
                    if (not primary_name) and legacy_name:
                        connection.execute(
                            ...
                            {"name": legacy_name, "id": primary_id},
                        )

                    if (not primary_current_add_id) and legacy_current_add_id:
                        normalized_current = (
                            legacy_current_add_id[4:]
                            if isinstance(legacy_current_add_id, str) and legacy_current_add_id.startswith('-100')
                            else legacy_current_add_id
                        )
                        connection.execute(
                            text("UPDATE chats SET current_add_id = :current_add_id WHERE id = :id"),
                            {"current_add_id": normalized_current, "id": primary_id},
                        )
                ...
                connection.execute(
                    text("UPDATE chats SET telegram_chat_id = :cid WHERE id = :id"),
                    {"cid": canonical_id, "id": chat_id},
                )
                normalized_count += 1

            # 同步 current_add_id 到统一格式
            connection.execute(
                text("UPDATE chats SET current_add_id = substr(current_add_id, 5) WHERE current_add_id LIKE '-100%'")
            )

            if merged_count or normalized_count:
                logging.info(
                    f'chats 频道ID标准化完成: 合并 {merged_count} 条，规范化 {normalized_count} 条'
                )
        except Exception as e:
            logging.error(f'标准化 chats 频道ID时出错: {str(e)}')
        ...
        try:
            ...
            if not index_exists:
                ...
                # 统一空值，保证唯一索引语义稳定
                connection.execute(text("UPDATE keywords SET is_regex = 0 WHERE is_regex IS NULL"))
                connection.execute(text("UPDATE keywords SET is_blacklist = 1 WHERE is_blacklist IS NULL"))

                # 清理重复数据，保留最早的 id，避免创建唯一索引失败
                dedup_result = connection.execute(text("""
                ...
                """))
                if dedup_result.rowcount and dedup_result.rowcount > 0:
                    logging.info(f'去重 keywords 重复记录: {dedup_result.rowcount} 条')

                connection.execute(text("""
                    CREATE UNIQUE INDEX IF NOT EXISTS unique_rule_keyword_is_regex_is_blacklist
