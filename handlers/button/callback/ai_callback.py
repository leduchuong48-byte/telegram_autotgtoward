...
async def get_dynamic_model_list() -> list:
    """尝试从 AI Provider 拉取实时模型，失败则返回默认列表"""
    try:
        ...
        logger.warning("Bot 拉取模型失败: %s", exc)
...
async def callback_set_ai_api_key(event, rule_id, session, message, data):
    rule = session.query(ForwardRule).get(rule_id)
    if not rule:
        await event.answer('规则不存在')
        return
    ...
    chat_id = abs(event.chat_id)
    state = f"set_ai_api_key:{rule_id}"
    state_manager.set_state(user_id, chat_id, state, message, state_type="ai")
    asyncio.create_task(cancel_state_after_timeout(user_id, chat_id))

    config = config_manager.get_config()
    ai_config = config.get("ai_service", {}) if isinstance(config, dict) else {}
    current_key = _mask_api_key(ai_config.get("api_key", ""))

    await message.edit(
        f"请发送新的 AI API Key\n"
        f"当前规则ID: `{rule_id}`\n"
        f"当前 Key：`{current_key}`\n\n"
        f"5分钟内未设置将自动取消",
        buttons=[[Button.inline("取消", f"cancel_set_ai_key:{rule_id}")]]
    )


async def callback_set_ai_provider(event, rule_id, session, message, data):
    buttons = []
    for provider, label in AI_PROVIDERS:
        buttons.append([Button.inline(label, f"select_ai_provider:{rule_id}:{provider}")])
    buttons.append([Button.inline("返回", f"ai_settings:{rule_id}")])
    await event.edit("请选择 AI 服务商：", buttons=buttons)


async def callback_select_ai_provider(event, rule_id, session, message, data):
    parts = data.split(':', 2)
    if len(parts) != 3:
        await event.answer("数据格式错误")
        return
    _, rule_id, provider = parts
    provider = provider.strip().lower()

    cfg = config_manager.get_config()
    ai_cfg = cfg.get("ai_service", {})
    if not isinstance(ai_cfg, dict):
        ai_cfg = {}
    ai_cfg["provider"] = provider
    cfg["ai_service"] = ai_cfg
    config_manager.update_config(cfg)

    rule = session.query(ForwardRule).get(int(rule_id))
    if rule:
        await event.edit(await get_ai_settings_text(rule), buttons=await create_ai_settings_buttons(rule))
    await event.answer("已更新 AI 服务商")
...
async def callback_model_page(event, rule_id, session, message, data):
    ...
    _, rule_id, page = data.split(':')
    page = int(page)
    await event.answer("正在刷新模型列表...")
    models = await get_dynamic_model_list()
    await event.edit("请选择AI模型：", buttons=await create_model_buttons(rule_id, page=page, models=models))
    return
...
async def callback_change_model(event, rule_id, session, message, data):
    await event.answer("正在刷新模型列表...")
    models = await get_dynamic_model_list()
    await event.edit("请选择AI模型：", buttons=await create_model_buttons(rule_id, page=0, models=models))
    return
...
async def callback_cancel_set_ai_key(event, rule_id, session, message, data):
    rule_id = data.split(':')[1]
    try:
        rule = session.query(ForwardRule).get(int(rule_id))
        if rule:
