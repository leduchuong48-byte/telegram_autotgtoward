...
import logging
from utils.common import get_media_settings_text, get_db_ops, is_admin
...
async def callback_set_max_media_size(event, rule_id, session, message, data):
        await event.edit("请选择媒体大小(MB)：", buttons=await create_media_size_buttons(rule_id, page=0))
        return
...
async def callback_set_custom_media_size(event, rule_id, session, message, data):
    rule = session.query(ForwardRule).get(int(rule_id))
    if not rule:
    ...
    state = f"set_custom_media_size:{rule_id}"

    try:
        state_manager.set_state(user_id, chat_id, state, message, state_type="media")
    ...
    await message.edit(
        f"请发送自定义媒体大小（单位MB，整数）\n"
        f"当前规则ID: `{rule_id}`\n"
        f"5分钟内未设置将自动取消",
        buttons=[[Button.inline("取消", f"cancel_set_media_custom:{rule_id}")]]
    )
    return


async def callback_set_media_duration(event, rule_id, session, message, data):
    await event.edit("请选择媒体时长(分钟)：", buttons=await create_media_duration_buttons(rule_id, page=0))
    return


async def callback_select_media_duration(event, rule_id, session, message, data):
    parts = data.split(':', 2)
    if len(parts) == 3:
        _, rule_id, minutes = parts
        logger.info(f"设置规则 {rule_id} 的媒体时长为: {minutes}")
        try:
            rule = session.query(ForwardRule).get(int(rule_id))
            if rule:
                old_minutes = rule.media_duration_minutes
                rule.media_duration_minutes = int(minutes)
                session.commit()
                logger.info(f"数据库更新成功: {old_minutes} -> {minutes}")

                if rule.enable_sync:
                    logger.info(f"规则 {rule.id} 启用了同步功能，正在同步媒体时长设置到关联规则")
                    sync_rules = session.query(RuleSync).filter(RuleSync.rule_id == rule.id).all()
                    for sync_rule in sync_rules:
                        sync_rule_id = sync_rule.sync_rule_id
                        logger.info(f"正在同步媒体时长到规则 {sync_rule_id}")
                        target_rule = session.query(ForwardRule).get(sync_rule_id)
                        if not target_rule:
                            logger.warning(f"同步目标规则 {sync_rule_id} 不存在，跳过")
                            continue
                        try:
                            old_target_minutes = target_rule.media_duration_minutes
                            target_rule.media_duration_minutes = int(minutes)
                            logger.info(
                                f"同步规则 {sync_rule_id} 的媒体时长从 {old_target_minutes} 到 {minutes}"
                            )
                        except Exception as e:
                            logger.error(f"同步媒体时长到规则 {sync_rule_id} 时出错: {str(e)}")
                            continue
                    session.commit()
                    logger.info("所有同步媒体时长更改已提交")

                await event.edit("媒体设置：", buttons=await create_media_settings_buttons(rule))
                await event.answer(f"已设置媒体时长为: {minutes}分钟")
        except Exception as e:
            logger.error(f"设置媒体时长时出错: {str(e)}")
            logger.error(f"错误详情: {traceback.format_exc()}")
        finally:
            session.close()
    return
...
async def callback_set_custom_media_duration(event, rule_id, session, message, data):
    rule = session.query(ForwardRule).get(int(rule_id))
    if not rule:
        await event.answer('规则不存在')
        return
    ...
    chat_id = abs(event.chat_id)
    state = f"set_custom_media_duration:{rule_id}"
...
async def callback_cancel_set_media_custom(event, rule_id, session, message, data):
...
async def callback_toggle_media_type(event, rule_id, session, message, data):
    ...
    try:
        ...
        media_type = parts[2]  
        # 检查媒体类型是否有效
        if media_type not in ['photo', 'document', 'video', 'audio', 'voice', 'text']:
