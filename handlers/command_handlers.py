...
from models.models import MediaTypes, MediaExtensions, ProcessedMessage
...
async def _pre_resolve_entity(bot_client, raw_input, entity, label):
    candidates = []
    ...
    if raw_input:
        add(raw_input)
        if isinstance(raw_input, str) and raw_input.startswith("t.me/"):
            add(f"https://{raw_input}")

    username = getattr(entity, "username", None)
    if username:
        add(f"@{username}")
        add(username)
        add(f"https://t.me/{username}")

    if isinstance(raw_input, str) and raw_input.isdigit():
        add(int(raw_input))
        add(int(f"-100{raw_input}"))
    ...
    for candidate in candidates:
        try:
            resolved = await bot_client.get_entity(candidate)
            title = getattr(resolved, "title", None) or getattr(resolved, "first_name", None) or str(resolved)
            logger.info(f"{label}预解析成功: 目标={candidate} ID={getattr(resolved, 'id', None)} 标题={title}")
            return
        except Exception as e:
            last_error = e

    logger.warning(f"{label}预解析失败: 目标={raw_input} err={str(last_error)}")


async def handle_backfill_command(event, client, parts):
    """
    回填历史消息（执行过滤器链：关键字/替换/媒体/AI/推送等）
    ...
    try:
        ...
        source_chat_name = source_chat.name
        target_chat_name = rule.target_chat.name if rule.target_chat else str(rule.target_chat_id)
    finally:
        session.close()
    ...
    try:
        ...
        if mode.isdigit():
            params = BackfillParams(mode="last", limit=int(mode), **throttle_kwargs)
        elif mode in ("last", "l"):
            if len(args) < 2 or not args[1].isdigit():
                raise ValueError("last 模式需要提供正整数 N")
            params = BackfillParams(mode="last", limit=int(args[1]), **throttle_kwargs)
            ...
            params = BackfillParams(mode="range", start_time=start_time, end_time=end_time, **throttle_kwargs)
        elif mode in ("all", "a"):
            params = BackfillParams(mode="all", **throttle_kwargs)
        else:
    ...
    await async_delete_user_message(event.client, event.message.chat_id, event.message.id, 0)
    await reply_and_delete(event, f"已开始回填：规则 {rule.id}（{source_chat_name} -> {target_chat_name}）\n停止：/backfill_stop")


async def handle_backfill_stop_command(event, client, parts):
    """停止当前聊天窗口的回填任务"""
    ok, msg, _ = await stop_backfill_task(notify_chat_id=event.chat_id)
    await async_delete_user_message(event.client, event.message.chat_id, event.message.id, 0)
    await reply_and_delete(event, msg)

async def handle_backfill_reset_command(event, client, parts):
    """清理当前规则的回填去重记录"""
    session = get_session()
    try:
        current = await get_current_rule(session, event)
        ...
        deleted = (
            ...
            .filter(ProcessedMessage.rule_id == rule.id)
        ...
        await reply_and_delete(
            event,
            f"已清理规则 {rule.id}（{source_chat.name}）的回填去重记录 {deleted} 条",
        )
    except Exception as e:
        session.rollback()
        logger.error(f'清理回填去重记录时出错: {str(e)}')
        await async_delete_user_message(event.client, event.message.chat_id, event.message.id, 0)
        await reply_and_delete(event, '清理回填去重记录失败，请检查日志')


async def handle_resolve_command(event, client, parts):
    """解析并缓存群组/频道实体"""
    args = parts[1:] if len(parts) > 1 else []
    if not args:
        await reply_and_delete(event, "用法: /resolve <群组链接|@用户名|ID>")
        return
    ...
    try:
        ...
        await reply_and_delete(event, f"✅ 已成功解析并缓存实体：{title} (ID: {getattr(entity, 'id', None)})")
        return
    except Exception as e:
        logger.warning(f"解析实体失败: target={target} err={str(e)}")
        if target.isdigit():
            try:
                ...
                await reply_and_delete(
                    event,
                    f"✅ 已成功解析并缓存实体：{title} (ID: {getattr(entity, 'id', None)})",
                )
                return
            except Exception as e2:
                logger.warning(f"解析实体失败(补前缀): target={target} err={str(e2)}")

        await reply_and_delete(event, f"解析失败: {str(e)}")
    finally:
        session.close()
...
async def handle_bind_command(event, client, parts):
    ...
    try:
        ...
        try:
            ...
            # 预解析 Bot 实体缓存，避免后续转发无法解析
            try:
                bot_client = await get_bot_client()
                await _pre_resolve_entity(bot_client, source_target, source_chat_entity, "源聊天")
                if target_chat_input:
                    await _pre_resolve_entity(bot_client, target_chat_input, target_chat_entity, "目标聊天")
                else:
                    await _pre_resolve_entity(bot_client, None, target_chat_entity, "目标聊天")
            except Exception as e:
                logger.warning(f"预解析 Bot 实体时出错: {str(e)}")
...
async def handle_clear_all_command(event):
    ...
    try:
        ...
        processed_count = session.query(models.ProcessedMessage).delete(synchronize_session=False)
...
async def handle_help_command(event, command):
    ...
    help_text = (
        ...
        "/backfill(/bf) <N|last|range|all...> - 回填历史消息（最近N/时间段/全部）\n"
...
async def handle_delete_rule_command(event, command, parts):
    ...
    try:
        ...
        for rule_id in ids_to_remove:
            ...
            try:
                # 清理幂等去重记录（避免残留/外键约束问题）
                session.query(models.ProcessedMessage).filter(
                    models.ProcessedMessage.rule_id == rule_id
                ).delete(synchronize_session=False)
